...
export async function generateBlogPost(data: PostFormValues, searchContext: string | undefined, apiKey: string, seoStrategy?: SEOStrategy, selectedModel: string = "gemini-3.5-flash"): Promise<string> {
...
export async function generateBlogImage(prompt: string, apiKey: string): Promise<string | null> {
    try {
        // Changed to use 3.1 image model
        const imageModel = getGeminiModel(apiKey, "gemini-3.1-flash-image");
...
export async function optimizeContent(content: string, suggestions: string[], apiKey: string): Promise<string> {
    try {
        const model = getGeminiModel(apiKey, "gemini-3.5-flash", 0.2);
